...
# `gaff check` reports that state. A merge queue or a direct push by
# GitHub runs no pre-push hook, so CI on the push event gates those.
set -e
...
if [ -z "${MERGE_GATE_SKIP_TESTS:-}" ] || [ -z "${CARGO:-}" ]; then
	...
	# shipped code. Without it the gate never compiles mcp. Capture the
	# output: on red, the failing test's name is the first thing a
	# reader needs, and it must reach the CI log.
