...
const SHORT: &str = "tisket";
/// The binary the build job archives.
const BIN: &str = "tisket";
...
fn every_npm_publish_path_is_relative() {
    ...
    // reads it as a folder. `npm publish npm/tisket-darwin-arm64` cloned
    // `github.com/npm/tisket-darwin-arm64` and failed on a key it has no
