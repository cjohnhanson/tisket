...
fn scratch_repo(name: &str) -> (PathBuf, String) {
    ...
    let repo =
        std::env::temp_dir().join(format!("tisket_review_gate_{}_{name}", std::process::id()));
    ...
    // would point this at the repository being tested.
    let git_dir = git(&repo, &["rev-parse", "--absolute-git-dir"]);
    // git resolves symlinks and the temp directory is one on macOS, so
    // both sides have to be canonical before they can be compared.
