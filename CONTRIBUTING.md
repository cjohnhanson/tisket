...
them with `cat`, or with tisket itself (`tisket issue list`,
`tisket issue show <id>`). Do not edit them in a pull request.
...
<https://pypi.org/manage/account/publishing/> with project `tisket`,
owner `cjohnhanson`, repository `tisket`, workflow `release.yml`, and
